...
#if ((F_FILE_NON_BLOCKING_WRITE | F_FILE_CLUSTER_ADDED |                       \
      F_FILE_UNBUFFERED_READ | F_FILE_DIR_DIRTY) &                             \
     F_OFLAG)
...
private:
  ...
  uint8_t isBusy(void) { return sdCard_->isBusy(); }
  uint8_t isCacheMirrorBlockDirty(void) { return (cacheMirrorBlock_ != 0); }
};
#endif // SdFat_h
